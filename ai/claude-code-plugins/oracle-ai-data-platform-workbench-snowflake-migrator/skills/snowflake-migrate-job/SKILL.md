---
name: snowflake-migrate-job
description: SCAFFOLD - NOT YET PORTED TO SNOWFLAKE, do not invoke for Snowflake work. (Original Databricks description follows.) Run the full Databricks→AIDP migration against a manifest. Pass-1 walks the %run dep tree and rewrites Databricks APIs in each dep notebook code-only. Pass-2 executes each task cell-by-cell on a live AIDP cluster, runs 4-way verify (exec error / stderr patterns / Spark logs / Opus eval), and re-attempts up to 10 times via Claude with tool use. Use when the user is ready to actually port the workload (not just plan it). Long-running — typical job takes 10–60 minutes per task depending on cell count.
---
...
# `snowflake-migrate-job` — execute the migration
...
- A valid manifest at `reports/<job>_manifest.json` (use [`snowflake-build-dag`](../snowflake-build-dag/SKILL.md)).
- A clean [`snowflake-check-data`](../snowflake-check-data/SKILL.md) (or an explicit "I know data is missing, proceed anyway" from the user).
...
| `--start-task <substring>` | Resume from this task (skip everything before). Pairs with [`snowflake-resume-migration`](../snowflake-resume-migration/SKILL.md). |
...
| Symptom | Skill / fix |
|---|---|
| `RESULT: PARTIAL` with N cells failing all 10 attempts | [`snowflake-fixup-cell`](../snowflake-fixup-cell/SKILL.md) for each. |
...
| User wants to resume after manual fixes to a dep | [`snowflake-resume-migration`](../snowflake-resume-migration/SKILL.md). |
...
- For any `PARTIAL` cells, route to [`snowflake-fixup-cell`](../snowflake-fixup-cell/SKILL.md).
- For streaming / batch convergence pipelines, follow up with [`snowflake-acceptance-contract`](../snowflake-acceptance-contract/SKILL.md).
