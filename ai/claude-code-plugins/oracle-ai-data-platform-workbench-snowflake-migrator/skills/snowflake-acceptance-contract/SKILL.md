---
name: snowflake-acceptance-contract
...
# `snowflake-acceptance-contract` — convergence verification for batch/streaming jobs
...
- Doesn't retry failed cells — it only signals "the pipeline didn't drain in time". For cell-level retries use [`snowflake-fixup-cell`](../snowflake-fixup-cell/SKILL.md).
