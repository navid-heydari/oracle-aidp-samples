...
# `snowflake-fixup-cell` — surgical re-execute of cells in a migrated notebook
...
- `JOB_REPORT.md` shows specific cells as `FAIL` after [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) finished.
...
| Cell K failed because the cluster died mid-execution | Restart the cluster, re-invoke [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) with `--skip-migrated` — fixup not needed. |
