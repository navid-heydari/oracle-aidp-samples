...
# `snowflake-migrate-catalog` — Unity Catalog / HMS → AIDP DDL migration
...
- BEFORE [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) for the first time on a new target environment.
- After the user adds new schemas/tables to the source Databricks workspace.
- After a [`snowflake-check-data`](../snowflake-check-data/SKILL.md) run reports many `MISSING` entries.
...
After the migration, verify schemas + tables actually landed via [`snowflake-check-data`](../snowflake-check-data/SKILL.md). Specifically:
...
- [`snowflake-bucket-mapping`](../snowflake-bucket-mapping/SKILL.md) if any tables still have `s3://` locations the bucket-map didn't cover.
- [`snowflake-check-data`](../snowflake-check-data/SKILL.md) to verify everything landed.
- Then [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) for the notebook layer.
