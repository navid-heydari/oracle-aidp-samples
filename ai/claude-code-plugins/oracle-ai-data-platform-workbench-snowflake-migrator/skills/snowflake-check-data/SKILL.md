---
name: snowflake-check-data
description: SCAFFOLD - NOT YET PORTED TO SNOWFLAKE, do not invoke for Snowflake work. (Original Databricks description follows.) Pre-migration data-availability scan. Reads every notebook in a migration manifest, extracts every spark.read.table / spark.read.parquet / saveAsTable reference, and probes whether each target schema/table/path exists on the AIDP cluster BEFORE you spend Pass-2 cluster time. Use after snowflake-build-dag and before snowflake-migrate-job, especially the first time you migrate against a target environment.
---
...
- After [`snowflake-build-dag`](../snowflake-build-dag/SKILL.md), before [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md).
- After [`snowflake-migrate-catalog`](../snowflake-migrate-catalog/SKILL.md) (verify schemas + tables actually landed).
...
Or for the workflow-shape input (matches [`snowflake-build-dag`](../snowflake-build-dag/SKILL.md)'s workflow path):
...
- Run [`snowflake-migrate-catalog`](../snowflake-migrate-catalog/SKILL.md) if the underlying *schema* is missing.
- Configure [`snowflake-bucket-mapping`](../snowflake-bucket-mapping/SKILL.md) if `s3://` → `oci://` rewrites haven't been done.
...
If the manifest references `s3://` paths, the scanner also consults `<migrator-repo>/config/oci_bucket_tenancy_mapping.json` (or whatever your bucket mapping helper resolves) to translate before probing. If the mapping is missing the bucket, the scanner reports a clear `S3 bucket X not found in OCI bucket mapping`. Fix via [`snowflake-bucket-mapping`](../snowflake-bucket-mapping/SKILL.md) and re-run.
...
If everything is OK: proceed to [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md).
If anything is MISSING: resolve via [`snowflake-migrate-catalog`](../snowflake-migrate-catalog/SKILL.md) or [`snowflake-bucket-mapping`](../snowflake-bucket-mapping/SKILL.md) and re-run this skill.
