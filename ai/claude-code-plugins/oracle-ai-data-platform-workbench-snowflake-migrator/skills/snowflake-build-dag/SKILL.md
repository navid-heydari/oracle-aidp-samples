...
# `snowflake-build-dag` — build the migration manifest
...
- Before any [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) invocation against a new workload.
...
1. Run [`snowflake-check-data`](../snowflake-check-data/SKILL.md) to verify source tables exist on the cluster.
2. Run [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) with `--manifest reports/<MyJob>_manifest.json`.
