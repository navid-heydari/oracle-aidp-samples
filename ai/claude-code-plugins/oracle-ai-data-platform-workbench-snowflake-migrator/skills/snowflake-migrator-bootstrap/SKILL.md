...
# `snowflake-migrator-bootstrap` — environment readiness check
...
- If the user is on a session token and it's about to expire (`oci session validate` shows <10 minutes remaining), warn them and suggest refresh BEFORE invoking a long-running skill like [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md).
