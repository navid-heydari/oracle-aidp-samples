---
name: snowflake-resume-migration
description: SCAFFOLD - NOT YET PORTED TO SNOWFLAKE, do not invoke for Snowflake work. (Original Databricks description follows.) Resume an interrupted migration. The migrator caches already-migrated notebooks (in-memory + on-cluster) so subsequent runs skip them. Use when a prior snowflake-migrate-job run was killed mid-flight (Ctrl-C, cluster restart, network drop) or when resuming after a manual fix to a specific dep notebook.
---
...
# `snowflake-resume-migration` — pick up where you left off
...
- A prior [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) run was killed (Ctrl-C, cluster restart, network drop, timeout).
...
- Same as [`snowflake-migrate-job`](../snowflake-migrate-job/SKILL.md) — read `JOB_REPORT.md`, route failed cells to [`snowflake-fixup-cell`](../snowflake-fixup-cell/SKILL.md), etc.
