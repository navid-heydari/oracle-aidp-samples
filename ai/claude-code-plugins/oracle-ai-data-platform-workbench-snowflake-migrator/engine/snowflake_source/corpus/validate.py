#!/usr/bin/env python3
"""Validate the Acme test corpus. PASS/FAIL per check, exit 0/1.
...
DB = os.environ.get("CORPUS_DB") or _migration_config().get(
...
def main() -> int:
    ...
    view_amt, fact_amt = one("""
        select (select sum(ITEM_TOTAL_AMOUNT) from ACME_ORDER_360_VW),
