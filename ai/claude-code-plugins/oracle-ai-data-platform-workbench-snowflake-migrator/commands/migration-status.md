---
description: SCAFFOLD - NOT YET PORTED TO SNOWFLAKE. Parse + summarize the JOB_REPORT.md produced by a prior migration run. Surfaces cell pass/fail/fix counts per task, lists the failing cells, and recommends a next-step skill (fixup-cell, resume, or all-clear).
---
...
Recommended actions:
  • <task_b>: cells 12 + 19 failed all 10 attempts.
    → Invoke snowflake-fixup-cell on each with a `why` reason.
  • <task_c>: structural failure (dep `helpers/io_utils` couldn't be migrated).
    → Open dep .ipynb manually, fix, then snowflake-resume-migration with --only-tasks <task_c>.
...
- After any [`snowflake-migrate-job`](../skills/snowflake-migrate-job/SKILL.md) run.
...
| PARTIAL with N failing cells | Route to [`snowflake-fixup-cell`](../skills/snowflake-fixup-cell/SKILL.md) for each. |
| FAIL on dep migration | Route to manual fix → [`snowflake-resume-migration`](../skills/snowflake-resume-migration/SKILL.md). |
| ACCEPTANCE_CONTRACT_VIOLATED | Investigate why the pending queue didn't drain. See [`snowflake-acceptance-contract`](../skills/snowflake-acceptance-contract/SKILL.md). |
